...
import com.sivalabs.bookstore.catalog.grpc.ProductValidationRequest;
import com.sivalabs.bookstore.catalog.grpc.ProductValidationResponse;
...
public class GrpcProductCatalogClient implements ProductCatalogPort {
    ...
    ProductValidationResponse validateProduct(String productCode, BigDecimal price) {
        try {
            ...
            return catalogStub.validateProduct(request);
    ...
    ProductValidationResponse handleValidationFailure(String productCode, BigDecimal price, Throwable throwable) {
        log.error("Catalog gRPC service unavailable for product {}: {}", productCode, throwable.getMessage());
